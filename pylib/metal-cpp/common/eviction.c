...
void evset_init(traverser_t ctraverser, int cevset_size, int cpage_size, int cmemory_size, int cthreshold) {
    ...
    if(!eviction_sets){
        eviction_sets = (Elem**)calloc(1, sizeof(Elem*));
    }
    
    if(!eviction_sets_pages){
        eviction_sets_pages = (uint64_t *)calloc(1, sizeof(char*));
    }
...
static Elem* initialize_list_with_offset(Elem* reference, uint64_t offset){
    ...
    uint64_t xo = ((uint64_t)reference & (page_size - 1)) ^ offset;
    Elem* last = NULL;
    Elem* start = (Elem*)((uint64_t)reference ^ xo);
    while(reference){
        Elem* cur = (Elem*)((uint64_t)reference ^ xo);
        cur->set = -2;
        cur->delta = 0;
        cur->prev = last;
        cur->next = NULL;
...
void evset_find(void* addr){
    ...
    if(same_page != -1){
        ...
        Elem* can = NULL;
        int retries = 0;
        while(gt_eviction(&start, &can, (char*)addr)){
            if(retries > 20){
    ...
    // Store the found eviction set for this address
...
uint64_t probe_cache_miss(){
    ...
    do {
        ...
        char* buf = (char*)malloc(PAGE_SIZE * 100);
...
cache_ctx_t cache_remove_prepare(char* addr){
    ...
    if(evsets_count % 8 == 1){
        eviction_sets = (Elem **)realloc(eviction_sets, sizeof(Elem*) * (evsets_count + 8));
        eviction_sets_pages = (uint64_t *)realloc(eviction_sets_pages, sizeof(uint64_t) * (evsets_count + 8));
    }
    evset_find(addr);
    
    timer_stop();
    return (cache_ctx_t)id;
